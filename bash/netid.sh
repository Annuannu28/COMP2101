...
while [ $# -gt 0 ]; do
    ...
    -v)
        verbose="yes"
        [ "$verbose" = "yes" ] && echo "Verbose mode turned on"
        ;;
    *)
        if [ "$userprovidedinterface" == "" ]; then
            userprovidedinterface="$1"
            [ "$verbose" = "yes" ] && echo "$userprovidedinterface details are requested"
        fi
        ;;
    esac
    shift
...
if [ "$userprovidedinterface" != "" ]; then
    interface=$userprovidedinterface
...
EOF
    ...
    [ "$verbose" = "yes" ] && echo "No specific interface details requeted"
fi
#####
...
#####
# Comments removed from the content above
INTERFACES=$(ifconfig | cut -d ' ' -f1 | tr ':' '\n' | awk NF)
[ "$verbose" = "yes" ] && echo "Gathering all interfaces in the system"
echo ""

for interface in $INTERFACES; do

  [ "$interface" = "lo" ] && echo "LO interface skipped just now" && continue
